...
const TimeTest = () => {
  ...
  const validateTimetable = (newEntry) => {
    ...
    const roomConflict = existingEntries.some(
      entry => entry.room === room && entry.day === day && (
    ...
    const lecturerConflict = existingEntries.some(
      entry => entry.lectureName === lectureName && entry.day === day && (
        (startTime >= entry.startTime && startTime < entry.endTime) ||
        (endTime > entry.startTime && endTime <= entry.endTime) ||
        (startTime <= entry.startTime && endTime >= entry.endTime)
      )
    ...
    const duplicateWeekday = existingEntries.some(entry => entry.day === day);
    ...
    if (duplicateWeekday) {
      setError("Only one entry per weekday is allowed.");
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ year: "", semester: "", moduleCode: "", moduleName: "", lectureName: "", room: "", startTime: "", endTime: "", day: "" });
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-white p-6 rounded-lg shadow-lg mb-6">
        <select name="year" value={form.year} onChange={handleChange} required className="border border-gray-300 p-2 rounded-lg">
          <option value="">Select Year</option>
          {[1, 2, 3, 4].map(year => <option key={year} value={year}>Year {year}</option>)}
        </select>
        <select name="semester" value={form.semester} onChange={handleChange} required className="border border-gray-300 p-2 rounded-lg">
          <option value="">Select Semester</option>
          {[1, 2].map(sem => <option key={sem} value={sem}>Semester {sem}</option>)}
        </select>
        <select name="moduleName" value={form.moduleName} onChange={handleChange} required className="border border-gray-300 p-2 rounded-lg">
          <option value="">Select Module</option>
          {courses.map(course => <option key={course._id} value={course.moduleName}>{course.moduleName}</option>)}
        </select>
        <input type="text" name="moduleCode" value={form.moduleCode} readOnly className="border border-gray-300 p-2 rounded-lg bg-gray-100" />
        <select name="lectureName" value={form.lectureName} onChange={handleChange} required className="border border-gray-300 p-2 rounded-lg">
          <option value="">Select Lecture</option>
          {lectures.map(lecture => <option key={lecture._id} value={lecture.lectureName}>{lecture.lectureName}</option>)}
        </select>
        <select name="room" value={form.room} onChange={handleChange} required className="border border-gray-300 p-2 rounded-lg">
          <option value="">Select Room</option>
          {rooms.map(room => <option key={room._id} value={room.roomName}>{room.roomName}</option>)}
        </select>
        <input
          type="time"
          name="startTime"
        ...
        <input
          ...
          className="border border-gray-300 p-2 rounded-lg"
        />
        <select name="day" value={form.day} onChange={handleChange} required className="border border-gray-300 p-2 rounded-lg">
          <option value="">Select Day</option>
          {["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"].map(day => <option key={day} value={day}>{day}</option>)}
        </select>
        <button type="submit" className="col-span-2 bg-blue-500 text-white py-2 rounded-lg shadow-md hover:bg-blue-600 transition duration-200">
          {editingId ? "Update Time Table" : "Add Time Table"}
        </button>
      ...
      {Object.entries(sortTimetablesByYearAndSemester(allTimeTables)).map(([key, timetables]) => (
        <div key={key} className="mt-8">
          ...
          <button
            onClick={() => generatePDFReport(timetables, key)}
              ...
              {sortTimetablesByDay(timetables).map(tt => (
                <tr key={tt._id} className="text-center">
                  ...
                  <td className="px-4 py-2 border">
                    <button onClick={() => handleEdit(tt._id)} className="mr-2 bg-yellow-500 px-2 py-1 text-white rounded">Edit</button>
                    <button onClick={() => handleDelete(tt._id)} className="bg-red-500 px-2 py-1 text-white rounded">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
